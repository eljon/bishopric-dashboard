...
// Geometry of the iOS-style spinning wheels.
...
function openWhenPicker(card, dateISO, timeHM) {
  ...
  whenState = {
    ...
    selDate: dateISO,
    selHour: hh,
    selMin: (Math.round(mm / 5) * 5) % 60, // snap to a 5-min slot
    dirty: false,
    summaryEl: null,
  };
  const backdrop = document.createElement('div');
  backdrop.id = 'when-backdrop';
  // Tapping the dimmed area (or Cancel/Escape) dismisses without committing.
  backdrop.addEventListener('click', (e) => { if (e.target === backdrop) closeWhenPicker(false); });
  const panel = buildWhenPanel();
  backdrop.appendChild(panel);
  document.body.appendChild(backdrop);
  // Center each wheel on its selected row once it's laid out.
  requestAnimationFrame(() => {
    panel.querySelectorAll('.wp-wheel').forEach(w => w._center && w._center());
  });
}
...
function buildWhenPanel() {
  ...
  // Toolbar: Cancel · live summary · Done (iOS action-sheet style).
  const bar = document.createElement('div');
  bar.className = 'wp-bar';
  const cancel = document.createElement('button');
  cancel.className = 'wp-btn wp-cancel'; cancel.textContent = 'Cancel';
  cancel.addEventListener('click', () => closeWhenPicker(false));
  const summary = document.createElement('div');
  summary.className = 'wp-summary';
  s.summaryEl = summary;
  const done = document.createElement('button');
  done.className = 'wp-btn wp-done'; done.textContent = 'Done';
  done.addEventListener('click', () => closeWhenPicker(true));
  bar.append(cancel, summary, done);
  panel.appendChild(bar);

  // Wheels: date · hour : minute, over a shared center selection band.
  ...
  const dateItems = buildDateItems(s.selDate);
  const dateWheel = buildWheel(dateItems, s.selDate, 'wp-wheel-date',
    (v) => { s.selDate = v; s.dirty = true; updateWhenSummary(); });

  const hourItems = [];
  for (let h = 0; h <= 23; h++) hourItems.push({ value: h, label: pad2(h) });
  const hourWheel = buildWheel(hourItems, s.selHour, 'wp-wheel-num',
    (v) => { s.selHour = v; s.dirty = true; updateWhenSummary(); });

  const minItems = [];
  for (let m = 0; m < 60; m += 5) minItems.push({ value: m, label: pad2(m) });
  const minWheel = buildWheel(minItems, s.selMin, 'wp-wheel-num',
    (v) => { s.selMin = v; s.dirty = true; updateWhenSummary(); });

  const colon = document.createElement('div');
  colon.className = 'wp-colon'; colon.textContent = ':';

  wheels.append(dateWheel, hourWheel, colon, minWheel);
...
// A range of days centered on today (with "Today" highlighted), always wide
// enough to include the currently selected date.
function buildDateItems(selISO) {
  const dayMs = 86400000;
  const today = new Date(); today.setHours(0, 0, 0, 0);
  const sel = new Date(selISO + 'T00:00:00'); sel.setHours(0, 0, 0, 0);
  const off = Math.round((sel - today) / dayMs);
  const start = Math.min(-120, off - 60);
  const end = Math.max(400, off + 60);
  const items = [];
  for (let o = start; o <= end; o++) {
    const d = new Date(today.getTime() + o * dayMs);
    items.push({
      value: toISODate(d),
      label: o === 0
        ? 'Today'
...
// Build one spinning wheel. `items` = [{value, label}]; `onSelect(value)` fires
// when the wheel settles on a new row. Returns the scroll element (with a
...
function updateWhenSummary() {
  const s = whenState;
  if (!s || !s.summaryEl) return;
  const d = new Date(s.selDate + 'T00:00:00');
  s.summaryEl.textContent =
    d.toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' }) +
    '  ·  ' + pad2(s.selHour) + ':' + pad2(s.selMin);
}

// Close the picker. Commit=true applies the selection (and triggers the board
// re-render + reorder animation) only now — not while picking.
function closeWhenPicker(commit) {
  ...
  if (commit && s && s.dirty) {
    ...
    s.card.interviewTime = pad2(s.selHour) + ':' + pad2(s.selMin);
    save();
    render();
  }
}
