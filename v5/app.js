...
function onCardPointerDown(e, card, list, li) {
  ...
  // Touch: arm a long-press. If the finger moves first, it's a scroll.
  if (e.pointerType === 'touch') {
    DRAG.pending.timer = setTimeout(() => { if (DRAG.pending) beginDrag(); }, 200);
  }
...
function onPointerMove(e) {
  ...
  if (!DRAG.active && !pend) return;
  const id = DRAG.active ? DRAG.active.pointerId : pend.pointerId;
  ...
  if (!DRAG.active) {
    ...
    if (pend.pointerType === 'touch') {
      // Moved before the long-press fired → treat as a scroll, cancel pickup.
      if (dist > 10 && pend.timer) { clearTimeout(pend.timer); cleanupPending(); }
      return;
    }
    if (dist > 6) beginDrag();
    else return;
  ...
  e.preventDefault(); // stop text selection / scrolling while dragging
...
function onPointerUp(e) {
  if (DRAG.active) { e.preventDefault(); endDrag(true); return; }
  const pend = DRAG.pending;
  if (pend && e.pointerId === pend.pointerId) {
    const dist = Math.hypot(e.clientX - pend.startX, e.clientY - pend.startY);
    const cardId = pend.card.id;
    cleanupPending();
    if (dist < 8) openCard(cardId); // it was a tap/click
  }
}
...
function beginDrag() {
  ...
  li.remove(); // remove the original; the placeholder represents its slot
...
function cardAfterPoint(ul, y) {
  const cards = $$('.card', ul); // placeholder is .card-placeholder, excluded
