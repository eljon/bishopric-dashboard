...
const DRAG = {
  ...
  active: null,    // { card, fromList, clone, placeholder, offsetX, offsetY, targetUl, arrows, inArrow }
  pointer: { x: 0, y: 0 },
  raf: null,
  arrowTimer: null, // repeats the prev/next-list switch while a finger rests on an arrow
};
...
function beginDrag() {
  ...
  // Left/right arrows that sit right beside the card and follow the finger:
  // nudge the card onto one to send it to the previous / next list. Visual only
  // (pointer-events:none); hit-tested by pointer coordinates in handleDragMove.
  const arrowL = document.createElement('div');
  arrowL.className = 'drag-arrow drag-arrow-left';
  arrowL.innerHTML = '<span>‹</span>';
  const arrowR = document.createElement('div');
  arrowR.className = 'drag-arrow drag-arrow-right';
  arrowR.innerHTML = '<span>›</span>';
  document.body.append(arrowL, arrowR);
  DRAG.active.arrows = { left: arrowL, right: arrowR };
  DRAG.active.inArrow = null;
  // Anchor the arrows near where the card was picked up (clamped on-screen).
  DRAG.active.anchorX = Math.max(ARROW_GAP + 26, Math.min(window.innerWidth - ARROW_GAP - 26, DRAG.pointer.x));
  positionArrows(DRAG.pointer.y);
  updateArrowDisabled();
...
function autoScrollTick() {
  ...
  // Horizontal board panning at the edges is replaced by the drag arrows, which
  // occupy those edges — so skip it while a drag (with arrows) is active.
  if (!(DRAG.active && DRAG.active.arrows)) {
...
function endDrag(commit) {
  ...
  a.clone.remove();
  if (a.arrows) { a.arrows.left.remove(); a.arrows.right.remove(); }
...
/* ---- Drag arrows: nudge a card to the previous / next list ---- */
const ARROW_GAP = 62; // px from the pickup point to each arrow (and the trigger threshold)

// Keep the arrows beside the pickup point, following the finger vertically.
function positionArrows(y) {
  const a = DRAG.active;
  if (!a || !a.arrows) return;
  a.arrows.left.style.left = (a.anchorX - ARROW_GAP) + 'px';
  a.arrows.left.style.top = y + 'px';
  a.arrows.right.style.left = (a.anchorX + ARROW_GAP) + 'px';
  a.arrows.right.style.top = y + 'px';
}

// One drag-move step: move the ghost + arrows, then either handle an arrow
// nudge or the normal in-list drop targeting.
function handleDragMove(x, y) {
  moveClone(x, y);
  positionArrows(y);
  const a = DRAG.active;
  const dx = a ? x - a.anchorX : 0;
  const zone = dx <= -ARROW_GAP ? 'left' : dx >= ARROW_GAP ? 'right' : null;
  updateArrowZone(zone);
  if (!zone) updateDropTarget(x, y); // only re-target by position when off the arrows
  ensureAutoScroll();
}

// Which list is the card currently headed for?
function currentTargetIndex() {
  const a = DRAG.active;
  const ul = (a && (a.targetUl || a.placeholder.parentElement)) || null;
  const id = ul && ul.dataset.listId;
  return board.lists.findIndex(l => l.id === id);
}

// Dim an arrow when there's no list that way.
function updateArrowDisabled() {
  const a = DRAG.active;
  if (!a || !a.arrows) return;
  const idx = currentTargetIndex();
  a.arrows.left.classList.toggle('off', idx <= 0);
  a.arrows.right.classList.toggle('off', idx < 0 || idx >= board.lists.length - 1);
}

function clearArrowTimer() {
  if (DRAG.arrowTimer) { clearInterval(DRAG.arrowTimer); DRAG.arrowTimer = null; }
}

// Handle entering / leaving an arrow zone. Entering switches once immediately,
// then repeats every 650ms while the finger stays on the arrow.
function updateArrowZone(zone) {
  const a = DRAG.active;
  if (!a || !a.arrows) return;
  a.arrows.left.classList.toggle('hot', zone === 'left');
  a.arrows.right.classList.toggle('hot', zone === 'right');
  if (zone === a.inArrow) return;
  a.inArrow = zone;
  clearArrowTimer();
  if (zone) {
    const dir = zone === 'left' ? -1 : 1;
    switchListByArrow(dir);
    DRAG.arrowTimer = setInterval(() => switchListByArrow(dir), 650);
  }
}

// Move the placeholder to the previous (dir=-1) / next (dir=+1) list.
function switchListByArrow(dir) {
  const a = DRAG.active;
  if (!a) return;
  const idx = currentTargetIndex();
  if (idx === -1) return;
  const nidx = idx + dir;
  if (nidx < 0 || nidx >= board.lists.length) { clearArrowTimer(); updateArrowDisabled(); return; }
  const ul = boardEl.querySelector('.cards[data-list-id="' + board.lists[nidx].id + '"]');
  if (!ul) return;
  a.targetUl = ul;
  ul.appendChild(a.placeholder); // drop at the end of the destination list
  const listEl = ul.closest('.list');
  if (listEl && listEl.scrollIntoView) listEl.scrollIntoView({ behavior: 'smooth', inline: 'center', block: 'nearest' });
