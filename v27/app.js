...
function openWhenPicker(card, dateISO, timeHM) {
  ...
  backdrop.addEventListener('click', (e) => { if (e.target === backdrop) closeWhenPicker(true); });
...
function refreshWhenPanel() {
  ...
  const [y, m, d] = s.selDate.split('-').map(Number);
  ...
  panel.appendChild(buildWhenTimeList());

  const done = document.createElement('button');
  done.className = 'wp-done';
  done.textContent = 'Done';
  done.addEventListener('click', () => closeWhenPicker(true));
  panel.appendChild(done);

  // Center the selected time in its scroller.
  const selEl = panel.querySelector('.wp-time-opt.sel');
...
function buildWhenCalendar() {
  ...
  for (let day = 1; day <= dim; day++) {
    ...
    b.textContent = day;
    b.addEventListener('click', () => { s.selDate = iso; s.dirty = true; refreshWhenPanel(); });
    grid.appendChild(b);
  }
...
function buildWhenTimeList() {
  ...
  listWrap.className = 'wp-time-list';
  for (let mins = 7 * 60; mins <= 21 * 60; mins += 5) { // 07:00–21:00 in 5-min steps
    const t = minToTime(mins);
    const b = document.createElement('button');
    b.className = 'wp-time-opt' + (t === s.selTime ? ' sel' : '');
    b.textContent = t;
    b.addEventListener('click', () => { s.selTime = t; s.dirty = true; refreshWhenPanel(); });
...
function closeWhenPicker(commit) {
  ...
  if (commit && s && s.dirty) {
    ...
    s.card.interviewTime = s.selTime;
    save();
    render();
  }
}
