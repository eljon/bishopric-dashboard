...
function openScheduleView() {
  closeScheduleView();
  const interviewList = board.lists.find(l => l.title === INTERVIEW_LIST);
  const cards = interviewList ? interviewList.cards.slice() : [];
  const schedule = computeInterviewSchedule(cards);
  ...
  title.textContent = 'Interview Schedule';
  ...
  function renderBody() {
    ...
    if (!visibleDates.length) {
      ...
      empty.textContent = selMember
        ? `No interviews for ${selMember}.`
        : 'No interviews scheduled yet. Add cards to “For Interview”.';
    ...
    for (const d of visibleDates) {
      ...
      dhCount.textContent = dayCards.length === 1 ? '1 interview' : `${dayCards.length} interviews`;
