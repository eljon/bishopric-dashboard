...
// Round a HH:MM time to the nearest 5 minutes.
function snapTo5(hhmm) {
  return minToTime(Math.round(timeToMin(hhmm) / 5) * 5);
}
  ...
  $$('#board .list').forEach(listEl => {
    ...
    $$('.card', listEl).forEach(c => {
      ...
      requestAnimationFrame(() => {
        c.style.transition = 'transform 0.34s cubic-bezier(0.22, 1, 0.36, 1)';
        c.style.transform = '';
      });
...
function renderCard(card, list, slotTime) {
  ...
  // 2-column badge (calendar | clock) with ONE datetime-local input overlaid,
  // so a single tap opens ONE combined date+time picker. The picked time is
  // snapped to the nearest 5 minutes (iOS shows a 1-minute wheel and ignores
  // `step`, so snapping is what actually enforces 5-minute increments there).
  if (list.title === INTERVIEW_LIST) {
    ...
    input.step = String(5 * 60); // 5-min on desktop (iOS ignores this; see snap below)
    ...
    input.addEventListener('change', () => {
      ...
      if (v) {
        ...
        card.interviewTime = snapTo5(t.slice(0, 5)); // enforce 5-minute increments
      }
      save(); render();
    });
    ['pointerdown', 'touchstart'].forEach(ev => input.addEventListener(ev, (e) => e.stopPropagation()));
    input.addEventListener('click', (e) => { e.stopPropagation(); try { input.showPicker(); } catch (_) {} });
