...
// 5-minute time options for the picker (07:00–21:00), always including `current`.
function timeOptions(current) {
  const set = new Set();
  for (let m = 7 * 60; m <= 21 * 60; m += 5) set.add(minToTime(m));
...
function renderCard(card, list, slotTime) {
  ...
  // Interview date & time (only in the "For Interview" list): a compact
  // 2-column badge with a native control overlaying each column —
  //   left (calendar)  → date input  (native date picker)
  //   right (clock)    → <select>    (native wheel of 5-minute times)
  // A <select> is used for time because iOS ignores the `step` attribute on
  // time/datetime inputs (it always shows 1-minute); a select renders as a
  // native wheel showing exactly the 5-minute options — so it works on mobile.
  if (list.title === INTERVIEW_LIST) {
    ...
    cal.innerHTML = `<span class="dt-head">${monthAbbr}</span><span class="dt-day">${D}</span>`;
    const dinput = document.createElement('input');
    dinput.type = 'date';
    dinput.value = dateISO;
    dinput.className = 'dt-input';
    dinput.setAttribute('aria-label', 'Interview date');
    dinput.addEventListener('change', () => { card.interviewDate = dinput.value || ''; save(); render(); });
    ['pointerdown', 'touchstart'].forEach(ev => dinput.addEventListener(ev, (e) => e.stopPropagation()));
    dinput.addEventListener('click', (e) => { e.stopPropagation(); try { dinput.showPicker(); } catch (_) {} });
    cal.appendChild(dinput);

    // Right column — time (5-minute wheel)
    const clock = document.createElement('span');
    clock.className = 'dt-clock';
    clock.title = 'Interview time — tap to change';
    clock.innerHTML = `<span class="dt-hh">${hh}</span><span class="dt-mm">${mm}</span>`;
    const tselect = document.createElement('select');
    tselect.className = 'dt-input';
    tselect.setAttribute('aria-label', 'Interview time');
    for (const t of timeOptions(timeHM)) {
      const o = document.createElement('option');
      o.value = t; o.textContent = t;
      if (t === timeHM) o.selected = true;
      tselect.appendChild(o);
    }
    tselect.addEventListener('change', () => { card.interviewTime = tselect.value; save(); render(); });
    ['pointerdown', 'touchstart', 'click'].forEach(ev => tselect.addEventListener(ev, (e) => e.stopPropagation()));
    clock.appendChild(tselect);

    dt.appendChild(cal);
    dt.appendChild(clock);
    top.appendChild(dt);
  }
