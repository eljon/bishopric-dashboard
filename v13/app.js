...
const ROSTER = ['Eljon Serrano', 'John Sombrero', 'Ace Magno', 'Arvin Navarra'];
  ...
  return {
    ...
    lists: [
      ...
      { id: uid(), title: 'For Discussion', cards: [
        mk('Nursery Teacher', { name: 'Krizia Cureg Dela Rosa', due: 3, checklist: cl(2) }),
        mk('Assistant Clerk - Records', { name: 'Jerson Danao', due: 1, watching: true, checklist: cl(2) }),
      ]},
      { id: uid(), title: 'Contacting', cards: [
        mk('Ward Mission Leader', { name: '', due: 1, description: 'Coordinate with elders quorum.', checklist: cl(2) }),
      ]},
      { id: uid(), title: 'For Interview', cards: [
        mk('Sunday School 2nd Counselor', { name: 'Kai Serrano', description: 'x', checklist: cl(2) }),
        mk('Relief Society Teacher', { name: 'Elisa Ruiz', checklist: cl(2) }),
        mk('Ward History Specialist', { name: 'Justine Matt', checklist: cl(2) }),
        mk('Sunday School Teacher', { name: 'Regine Villaruel', due: 1, checklist: cl(2) }),
        mk('Gatherers of Light 2nd Counselor', { name: 'Czarina Trinidad', due: 1, watching: true, checklist: cl(2) }),
        mk('Young Women Teacher', { name: 'Mean Magalang', due: 2, checklist: cl(2) }),
      ...
      { id: uid(), title: 'For Sustaining', cards: [
        mk('Disability Specialist', { name: 'Kathleen Barboza', due: 1, checklist: cl(2) }),
        mk('Relief Society Teacher', { name: 'Jasmin Serrano', checklist: cl(2) }),
      ]},
      { id: uid(), title: 'For Setting Apart', cards: [] },
      { id: uid(), title: 'For Releasing', cards: [] },
      { id: uid(), title: 'For Recording', cards: [
        mk('Priests\' Quorum 2nd Assistant', { name: 'Joven Cris Matt', due: 3, checklist: cl(2) }),
        mk('Teachers\' Quorum 2nd Counselor', { name: 'Jethro Moran', checklist: cl(2) }),
      ]},
      { id: uid(), title: 'Done', cards: [
        mk('Young Men Adviser', { name: 'John Robin Ayo', due: 5, checklist: cl(3) }),
        mk('Teachers\' Quorum Secretary', { name: 'Leo Domingo', due: 5, checklist: cl(3) }),
        mk('Teachers\' Quorum 1st Counselor', { name: 'Neil Navarra', checklist: cl(2) }),
        mk('YSA Leader', { name: 'Trisia Talosig', due: 2, checklist: cl(2) }),
        mk('Sunday School President', { name: 'Neil Navarra', due: 2, checklist: cl(2) }),
        mk('Welfare & Self-Reliance Leader', { name: 'Hela Panay', due: 2, checklist: cl(2) }),
        mk('RS Ministering Secretary', { name: 'Yolly Matuguinas', due: 2, checklist: cl(2) }),
...
function normalize(data) {
  ...
  data.lists = data.lists.map(l => ({
    ...
    cards: (l.cards || []).map(c => ({
      ...
      // Single assignee. Cleared once when migrating an older board; a new
      // board falls back to its first legacy member if present.
...
function renderCard(card, list) {
  ...
  badges.appendChild(assignBtn);

  li.appendChild(badges); // always shown — every card has an assignee slot
  ...
  const commit = () => {
    ...
    if (val) {
      list.cards.push({ id: uid(), position: val, name: '', due: null,
        watching: false, description: '', labels: [], checklist: [] });
      save(); render();
    } else close();
  };
