...
function beginDrag() {
  ...
  // Big left/right arrows: drag the card onto one to send it to the previous /
  // next list. They're visual only (pointer-events:none); hit-testing is by
  // pointer coordinates in handleDragMove.
...
/* ---- Drag arrows: fling a card to the previous / next list ---- */
const ARROW_ZONE = 74; // px hit-band on each screen edge

// One drag-move step: move the ghost, then either handle an arrow zone or the
// normal in-list drop targeting.
function handleDragMove(x, y) {
  moveClone(x, y);
  const zone = (x <= ARROW_ZONE) ? 'left' : (x >= window.innerWidth - ARROW_ZONE) ? 'right' : null;
