...
const SCHEMA = 11;
// Bump to force a one-time re-seed of the local board (e.g. after updating the
// seed to match the current Trello state).
const SEED_REV = 4;

/* Lists whose cards get a scheduled date + time (badge, auto-scheduling,
   date-sorting, and the Schedule view). */
const INTERVIEW_LIST = 'For Interview';
const SCHEDULED_LISTS = ['Contacting', 'For Interview'];
const isScheduledList = (title) => SCHEDULED_LISTS.includes(title);
...
// assignee + date and given 20-minute slots from 11:00. No two cards in a group
// ever share a slot — a manually-picked time is honored as a preference, but if
// it collides with another card it's bumped to the next free slot; cards without
// a pick fill the earliest remaining slots. Returns a Map of cardId -> "HH:MM".
function computeInterviewSchedule(cards) {
  ...
  for (const gcards of groups.values()) {
    const taken = new Set();
    // Pass 1: honor manual picks (in list order), but never double-book the same
    // person — a colliding pick is bumped forward to the next free 20-min slot.
    for (const c of gcards) {
      if (!c.interviewTime) continue;
      let slot = timeToMin(c.interviewTime);
      while (taken.has(slot)) slot += SLOT_MIN;
      taken.add(slot);
      result.set(c.id, minToTime(slot));
    }
    // Pass 2: cards without a pick fill the earliest free slots from 11:00.
  ...
  return {
    ...
    lists: [
      ...
      { id: uid(), title: 'For Interview', cards: [
        ...
        mk('RS Assistant Activity Coordinator', { name: 'Judith Quetua' }),
        mk('Ward Missionary', { name: 'Sister Geronimo' }),
      ]},
      { id: uid(), title: 'For Sustaining', cards: [
        mk('Sunday School Teacher', { name: 'Elisa Ruiz' }),
        mk('Deacons\' Quorum Secretary', { name: 'Elwin Zaldivar' }),
        mk('EnglishConnect Teacher', { name: 'Abram Serrano' }),
      ]},
      { id: uid(), title: 'For Setting Apart', cards: [
        mk('Disability Specialist', { name: 'Jidilea Serrano' }),
        mk('Sunday School Teacher', { name: 'Pao Sombrero' }),
        mk('Assistant YSA Leader', { name: 'Dennise Honrado' }),
      ]},
      { id: uid(), title: 'For Releasing', cards: [] },
      { id: uid(), title: 'For Recording', cards: [
        mk('Teachers\' Quorum Assistant Secretary', { name: 'Kyle Semine' }),
        mk('WSR Community Resources Specialist', { name: 'Virgie Villaruel' }),
        mk('Sunday School 2nd Counselor', { name: 'Kai Serrano' }),
        mk('Sunday School 1st Counselor', { name: 'Josephine Talosig' }),
        mk('Temple Prep. Teacher', { name: 'Griselda Tolones' }),
        mk('WSR Professions Specialist', { name: 'Jasmine Bensig' }),
        mk('WSR Professions Specialist', { name: 'Doll Almanon' }),
        mk('Sunday School Teacher', { name: 'Janah Serrano' }),
        mk('Sunday School Teacher', { name: 'Tina Baradi' }),
        mk('Sunday School Assistant Secretary', { name: 'Leonora Destura' }),
      ]},
...
function loadLocal() {
  try {
    ...
    if (raw) {
      ...
      // Re-seed once when the seed revision changes (e.g. a board content update).
      if (data && Array.isArray(data.lists) && data.seedRev === SEED_REV) return normalize(data);
    }
...
function normalize(data) {
  ...
  data.lists = data.lists.map(l => ({
    ...
    cards: (l.cards || []).map(c => ({
      ...
      forReleasing: !!c.forReleasing,       // shows a "Release" badge on the card
      // due / watching / checklist were removed — intentionally dropped here.
    })),
  }));
  data.schema = SCHEMA;
  data.seedRev = SEED_REV; // stamp so a normalized/remote board isn't re-seeded next load
  return data;
}
...
function render() {
  ...
  const prevScrollLeft = boardEl.scrollLeft; // keep the view on the same list
  ...
  boardEl.scrollLeft = prevScrollLeft; // restore horizontal focus after rebuild
...
function captureAutoSortRects() {
  ...
  $$('#board .list').forEach(listEl => {
    if (!isScheduledList(listEl.querySelector('.list-title')?.value)) return;
...
function renderCard(card, list, slotTime) {
  ...
  // A note indicator when the card has any notes/description.
  if ((card.description || '').trim()) {
    const note = document.createElement('span');
    note.className = 'card-note';
    note.title = 'Has notes';
    note.setAttribute('aria-label', 'Has notes');
    note.innerHTML = iconNote();
    heading.appendChild(note);
  }

  top.appendChild(heading);

  // Right-side tools: a row with the interview calendar badge (For Interview
  // only) + the assignee icon, and — when the card is marked for releasing — a
  // single "Release" badge spanning underneath BOTH icons (so neither icon
  // shifts when the badge toggles).
  const tools = document.createElement('div');
  tools.className = 'card-tools';
  const toolsRow = document.createElement('div');
  toolsRow.className = 'card-tools-row';
  ...
  toolsRow.appendChild(assignBtn);

  tools.appendChild(toolsRow);
  if (card.forReleasing) {
    const rel = document.createElement('span');
    rel.className = 'release-badge';
    rel.textContent = 'Release';
    rel.title = 'Marked for releasing';
    tools.appendChild(rel);
  }
...
/* ----------------------- Schedule (calendar) view -----------------------
   A read-only agenda of the "For Interview" cards grouped by their date. Since
   interviews are held on Sundays, this reads as a per-Sunday schedule: each
   date is a section, listing that day's interviews in time order. */
function closeScheduleView() {
  const bd = document.getElementById('sched-backdrop');
  if (bd) bd.remove();
}
function openScheduleView() {
  closeScheduleView();
  // Cards from every scheduled list, each list keeping its own conflict-free
  // schedule; remember which list each card came from for a per-row tag.
  const schedule = new Map();
  const listOf = new Map();
  const cards = [];
  board.lists.filter(l => isScheduledList(l.title)).forEach(l => {
    computeInterviewSchedule(l.cards).forEach((v, k) => schedule.set(k, v));
    l.cards.forEach(c => { listOf.set(c.id, l.title); cards.push(c); });
  });

  // Group by date, then order dates ascending and each day's cards by time.
  const byDate = new Map();
  for (const c of cards) {
    const d = effectiveInterviewDate(c);
    (byDate.get(d) || byDate.set(d, []).get(d)).push(c);
  }
  const dates = [...byDate.keys()].sort();
  for (const d of dates) {
    byDate.get(d).sort((a, b) => (schedule.get(a.id) || '').localeCompare(schedule.get(b.id) || ''));
  }

  const backdrop = document.createElement('div');
  backdrop.id = 'sched-backdrop';
  backdrop.addEventListener('click', (e) => { if (e.target === backdrop) closeScheduleView(); });
  ...
  close.className = 'sched-close';
  close.setAttribute('aria-label', 'Close');
  close.innerHTML = '&times;';
  close.addEventListener('click', closeScheduleView);
  head.append(title, close);
  panel.appendChild(head);

  // Member filter: tap an avatar to show only that person's interviews; tap it
  // again to clear. Selecting one deselects the rest.
  let selMember = null;
  const memberRow = document.createElement('div');
  memberRow.className = 'sched-members';
  const memberBtns = {};
  board.members.forEach(name => {
    const btn = document.createElement('button');
    btn.className = 'sched-member';
    btn.title = name;
    btn.setAttribute('aria-label', 'Filter to ' + name);
    btn.appendChild(avatarEl(name));
    btn.addEventListener('click', () => {
      selMember = (selMember === name) ? null : name;
      board.members.forEach(n => memberBtns[n].classList.toggle('sel', selMember === n));
      memberRow.classList.toggle('filtering', selMember !== null);
      renderBody();
    });
    memberBtns[name] = btn;
  ...
  function renderBody() {
    body.innerHTML = '';
    const match = c => !selMember || c.assignee === selMember;
    const visibleDates = dates.filter(d => byDate.get(d).some(match));

    if (!visibleDates.length) {
      const empty = document.createElement('div');
      empty.className = 'sched-empty';
      empty.textContent = selMember
        ? `Nothing scheduled for ${selMember}.`
        : 'Nothing scheduled yet. Add cards to “Contacting” or “For Interview”.';
      body.appendChild(empty);
      return;
    }

    for (const d of visibleDates) {
      const dayCards = byDate.get(d).filter(match);
      const [Y, M, D] = d.split('-').map(Number);
      const dateObj = new Date(Y, M - 1, D);
      const daySec = document.createElement('div');
      daySec.className = 'sched-day';

      const dh = document.createElement('div');
      dh.className = 'sched-day-head';
      const dhDate = document.createElement('span');
      dhDate.className = 'sched-day-date';
      dhDate.textContent = dateObj.toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' });
      const dhCount = document.createElement('span');
      dhCount.className = 'sched-day-count';
      dhCount.textContent = dayCards.length === 1 ? '1 scheduled' : `${dayCards.length} scheduled`;
      dh.append(dhDate, dhCount);
      daySec.appendChild(dh);

      dayCards.forEach(card => {
        const row = document.createElement('button');
        row.className = 'sched-row';
        row.addEventListener('click', () => { closeScheduleView(); openCard(card.id); });

        const time = document.createElement('span');
        time.className = 'sched-time';
        time.textContent = schedule.get(card.id) || effectiveInterviewTime(card);

        const info = document.createElement('div');
        info.className = 'sched-info';
        const nm = document.createElement('div');
        nm.className = 'sched-name' + (card.name ? '' : ' unassigned');
        nm.textContent = card.name || 'Unassigned';
        const cal = document.createElement('div');
        cal.className = 'sched-calling';
        cal.textContent = card.position;
        const listTitle = listOf.get(card.id);
        const confirmed = listTitle === INTERVIEW_LIST;
        const tag = document.createElement('span');
        tag.className = 'sched-list-tag' + (confirmed ? ' confirmed' : '');
        tag.textContent = confirmed ? 'Confirmed' : listTitle;
        info.append(nm, cal, tag);

        const who = document.createElement('div');
        who.className = 'sched-who';
        if (card.assignee) who.appendChild(avatarEl(card.assignee));
        if (card.forReleasing) {
          const rel = document.createElement('span');
          rel.className = 'release-badge';
          rel.textContent = 'Release';
  ...
  backdrop.appendChild(panel);
  document.body.appendChild(backdrop);
  enableSchedSwipe(panel, body);
}

// Pull-to-dismiss for the schedule bottom sheet: drag down from the handle or
// the top of the (unscrolled) list to close. Past a threshold or a quick flick
// it dismisses; otherwise it snaps back.
function enableSchedSwipe(panel, body) {
  let startY = 0, dy = 0, startT = 0, tracking = false, dragging = false;
  const now = () => (typeof performance !== 'undefined' ? performance.now() : 0);

  panel.addEventListener('touchstart', (e) => {
    if (e.touches.length !== 1 || body.scrollTop > 0) { tracking = false; return; }
  ...
  panel.addEventListener('touchmove', (e) => {
    ...
    if (!dragging) {
      if (delta <= 0 || body.scrollTop > 0) { tracking = false; return; } // it's a scroll
      dragging = true;
    }
    dy = Math.max(0, delta);
  ...
  const end = () => {
    ...
    if (dy > 120 || (dy > 40 && dt < 250)) {
      ...
      setTimeout(closeScheduleView, 190);
    } else {
...
if (calBtn) {
  calBtn.addEventListener('click', openScheduleView);
  // On phones the button floats at the bottom. The top bar has backdrop-filter,
  // which would trap a position:fixed child inside the header — so move the
  // button to <body> on mobile (and back into the header on desktop).
  const calHome = calBtn.parentElement; // the top bar
  const calMq = window.matchMedia('(max-width: 640px)');
  const placeCalBtn = () => {
    if (calMq.matches) { if (calBtn.parentElement !== document.body) document.body.appendChild(calBtn); }
    else if (calBtn.parentElement !== calHome) { calHome.appendChild(calBtn); }
  };
  calMq.addEventListener('change', placeCalBtn);
  placeCalBtn();
}
...
const DRAG = {
  ...
  active: null,    // { card, fromList, clone, placeholder, offsetX, offsetY, targetUl, samples }
  pointer: { x: 0, y: 0 },
  raf: null,
};
...
function beginDrag() {
  ...
  DRAG.active.samples = [{ x: DRAG.pointer.x, y: DRAG.pointer.y, t: performance.now() }];
  DRAG.active.startScrollLeft = boardEl.scrollLeft; // to keep the view put after a flick
...
function endDrag(commit) {
  ...
  // A quick horizontal flick sends the card to the adjacent list (and flies it
  // off that way), as long as there is a list in that direction.
  if (commit) {
    const dir = detectFlick(a);
    const idx = board.lists.indexOf(a.fromList);
    if (dir !== 0 && idx + dir >= 0 && idx + dir < board.lists.length) {
      flickCardAway(a, dir);
      return;
    }
  }
...
/* ---- Flick a card to the previous / next list ----
   While holding a card, a quick horizontal flick sends it to the adjacent list:
   the card flies off in that direction and lands in the prev/next list. The
   board view stays put (we never scroll to the destination). */
const FLICK_V = 0.55;   // px/ms — minimum horizontal speed to count as a flick

// One drag-move step: move the ghost, record a velocity sample, keep the in-list
// drop target updated.
function handleDragMove(x, y) {
  moveClone(x, y);
  const a = DRAG.active;
  if (a) {
    a.samples.push({ x, y, t: performance.now() });
    if (a.samples.length > 6) a.samples.shift();
  }
  updateDropTarget(x, y);
  ensureAutoScroll();
}

// If the release was a quick horizontal flick, return its direction (-1 left,
// +1 right); otherwise 0.
function detectFlick(a) {
  const s = a.samples;
  if (!s || s.length < 2) return 0;
  const last = s[s.length - 1];
  let ref = s[0];
  for (let i = s.length - 1; i >= 0; i--) { if (last.t - s[i].t >= 55) { ref = s[i]; break; } }
  const dt = last.t - ref.t;
  if (dt <= 0) return 0;
  const vx = (last.x - ref.x) / dt;
  const vy = (last.y - ref.y) / dt;
  if (Math.abs(vx) < FLICK_V) return 0;          // not fast enough
  if (Math.abs(vx) < Math.abs(vy) * 1.1) return 0; // mostly vertical → not a side flick
  return vx > 0 ? 1 : -1;
}

// Commit the card to the prev/next list and animate the clone flying off that
// way. Does NOT scroll the board — focus stays on the current list.
function flickCardAway(a, dir) {
  const toList = board.lists[board.lists.indexOf(a.fromList) + dir];
  a.fromList.cards = a.fromList.cards.filter(c => c.id !== a.card.id);
  toList.cards.push(a.card);
  save();

  const clone = a.clone;
  const fly = dir > 0 ? (window.innerWidth - clone.getBoundingClientRect().left + 40)
                      : -(clone.getBoundingClientRect().right + 40);
  clone.style.transition = 'transform 0.7s cubic-bezier(0.3, 0.7, 0.3, 1), opacity 0.7s ease-out';
  clone.style.transform = `translateX(${fly}px) rotate(${dir * 6}deg) scale(0.9)`;
  ...
  suppressReorderAnim = true;
  render(); // current list loses the card; the flying clone keeps animating on top
  boardEl.scrollLeft = a.startScrollLeft; // undo any edge auto-scroll drift — stay on the current list

  // The card "lands" in the destination list: wiggle + glow so it's clear it
  // arrived (visible on desktop and when you swipe over to that list). Timed to
  // roughly when the flying clone reaches the list.
  const landedId = a.card.id;
  setTimeout(() => {
    const landed = boardEl.querySelector('.card[data-card-id="' + landedId + '"]');
    if (!landed) return;
    landed.classList.add('just-landed');
    landed.addEventListener('animationend', () => landed.classList.remove('just-landed'), { once: true });
  }, 580);
}
...
function openCardComposer(list, ul, addBtn) {
  ...
  // One card-shaped composer: a bold Name over a muted Calling, mirroring the
  // card front (title over subtitle). We work hard to stop the browser
  // prefilling these from previous entries — iOS Safari ignores
  // autocomplete="off", so we ALSO give each field a random name and start it
  // `readonly` (autofill skips readonly fields), unlocking it on first focus.
  const rnd = Math.random().toString(36).slice(2);
  wrap.innerHTML = `
    <div class="composer-card">
      <input type="text" class="composer-name" placeholder="Name" name="n-${rnd}" readonly
             autocomplete="off" autocorrect="off" autocapitalize="words" spellcheck="false" />
      <input type="text" class="composer-calling" placeholder="Calling" name="c-${rnd}" readonly
             autocomplete="off" autocorrect="off" autocapitalize="words" spellcheck="false" />
    </div>
  ...
  const nameEl = $('.composer-name', wrap);
  const callingEl = $('.composer-calling', wrap);
  // Unlock (allow typing) on any real interaction — the fields are only
  // readonly to skip the browser's on-load autofill. Also unlock via a short
  // safety timeout so a field can never get stuck read-only.
  [nameEl, callingEl].forEach(el => {
  ...
  setTimeout(() => { nameEl.removeAttribute('readonly'); callingEl.removeAttribute('readonly'); }, 350);
  nameEl.focus(); // fires focus → unlocks the name field immediately
  const close = () => { wrap.remove(); addBtn.style.display = ''; };
  const commit = () => {
    const name = nameEl.value.trim();
    const calling = callingEl.value.trim();
    if (name || calling) {
      list.cards.push({ id: uid(), position: calling, name, assignee: '', description: '', labels: [] });
      save(); render();
    } else close();
  };
  ...
  const onKey = (e) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      // Enter in the name field advances to the calling field; Enter there commits.
      if (e.target === nameEl && !callingEl.value.trim()) callingEl.focus();
      else commit();
    }
    if (e.key === 'Escape') close();
  };
  nameEl.addEventListener('keydown', onKey);
  callingEl.addEventListener('keydown', onKey);
}
...
function openCard(cardId) {
  ...
  // Calling / position — editable field (no section label, per request).
  const posRow = document.createElement('div');
  posRow.className = 'chip-input';
  const posInput = document.createElement('input');
  posInput.type = 'text';
  posInput.placeholder = 'Calling / position';
  posInput.value = card.position;
  posInput.setAttribute('autocomplete', 'off');
  posInput.addEventListener('change', () => {
    card.position = posInput.value.trim() || 'Untitled';
    save(); render();
  });
  posRow.appendChild(posInput);
  modal.appendChild(posRow);

  // For releasing (adds a "Release" badge to the card) — above the list picker.
  const relSec = section('Status');
  const relRow = document.createElement('label');
  relRow.className = 'check-row';
  const relBox = document.createElement('input');
  relBox.type = 'checkbox';
  relBox.checked = !!card.forReleasing;
  relBox.addEventListener('change', () => {
    card.forReleasing = relBox.checked;
    save(); render();
  });
  const relText = document.createElement('span');
  relText.textContent = 'For releasing';
  relRow.append(relBox, relText);
  relSec.appendChild(relRow);
  modal.appendChild(relSec);
  ...
  // Notes (a card with any notes shows a note icon on the compact card).
  const notesSec = section('Notes');
  const notes = document.createElement('textarea');
  notes.className = 'desc';
  notes.placeholder = 'Add notes about this calling…';
  notes.value = card.description;
  notes.addEventListener('change', () => { card.description = notes.value.trim(); save(); render(); });
...
function iconNote() { return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M5 5h14M5 10h14M5 15h9"/></svg>'; }
...
// One-time content refresh of the shared board: when its seed revision is old,
// replace the pipeline lists with the current seed's cards (keeping
// Proposal/Pending and Done as they are on the shared board), so a seed update
// actually reaches everyone. Returns true if it changed anything.
const SEED_REFRESH_LISTS = new Set([
  'For Discussion', 'Contacting', 'For Interview', 'For Sustaining',
  'For Setting Apart', 'For Releasing', 'For Recording',
]);
function applySeedRefresh(b) {
  const fresh = seed();
  const freshByTitle = Object.fromEntries(fresh.lists.map(l => [l.title, l]));
  b.lists = b.lists.map(l =>
    (SEED_REFRESH_LISTS.has(l.title) && freshByTitle[l.title])
      ? { ...l, cards: freshByTitle[l.title].cards }
      : l);
  b.seedRev = SEED_REV;
  return b;
}
...
function onRemoteSnapshot(snap) {
  ...
  // Did the shared board predate the current seed revision? Refresh its pipeline
  // lists from the new seed and push it back for everyone.
  const needsSeedRefresh = data.board.seedRev !== SEED_REV;
  ...
  if (needsSeedRefresh) {
    board = applySeedRefresh(incoming);
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(toStorage(board))); } catch (_) {}
    scheduleRemotePush(true); // write the refreshed board back to Firebase
