...
function renderCard(card, list) {
  ...
  // Interview date & time (only in the "For Interview" list): one combined
  // event-style badge — red month header, then day + time — with a single
  // datetime-local input overlaid, so ONE tap opens ONE picker for both date
  // and time. Defaults: upcoming Sunday, 11:00 AM.
  if (list.title === INTERVIEW_LIST) {
    const dateISO = effectiveInterviewDate(card);
    const timeHM = effectiveInterviewTime(card);
    const [Y, M, D] = dateISO.split('-').map(Number);
    const monthAbbr = new Date(Y, M - 1, D).toLocaleDateString(undefined, { month: 'short' }).toUpperCase();

    const [hh, mm] = timeHM.split(':'); // 24-hour

    const dt = document.createElement('div');
    dt.className = 'dt-badge';
    dt.title = 'Interview date & time — tap to change';
    dt.innerHTML =
      `<span class="dt-cal"><span class="dt-head">${monthAbbr}</span>` +
      `<span class="dt-day">${D}</span></span>` +
      `<span class="dt-clock"><span class="dt-hh">${hh}</span>` +
      `<span class="dt-mm">${mm}</span></span>`;

    const input = document.createElement('input');
    input.type = 'datetime-local';
    input.value = `${dateISO}T${timeHM}`;
    input.className = 'dt-input';
    input.setAttribute('aria-label', 'Interview date and time');
    input.addEventListener('change', () => {
      const v = input.value; // "YYYY-MM-DDTHH:MM"
      if (v) {
        const [d, t] = v.split('T');
        card.interviewDate = d;
        card.interviewTime = (t || '').slice(0, 5);
      }
      save(); render();
    });
    ['pointerdown', 'touchstart'].forEach(ev =>
      input.addEventListener(ev, (e) => e.stopPropagation())); // don't drag/open the card
    input.addEventListener('click', (e) => {
      e.stopPropagation();
      try { input.showPicker(); } catch (_) {} // tap already opens on mobile; this helps desktop
    });
