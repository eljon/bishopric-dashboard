...
function renderCard(card, list) {
  ...
  // Interview date "app icon" (only in the "For Interview" list): a mini
  // macOS-Calendar-style icon (red month header + big day number). Defaults to
  // the upcoming Sunday; tap to pick another date via the native picker.
  if (list.title === INTERVIEW_LIST) {
    ...
    const openPicker = (e) => {
      e.stopPropagation();
      e.preventDefault();
      try { dinput.showPicker(); }        // reliable across desktop + mobile
      catch (_) { dinput.focus(); dinput.click(); } // fallback for old browsers
    };
    cal.addEventListener('click', openPicker);
    cal.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') openPicker(e); });
    cal.addEventListener('pointerdown', (e) => e.stopPropagation()); // don't start a card drag
    top.appendChild(cal);
  }
