...
// Default interview time when none is set yet.
...
function renderList(list) {
  ...
  if (list.title === INTERVIEW_LIST) {
    const key = c => effectiveInterviewDate(c) + 'T' + effectiveInterviewTime(c);
    list.cards.sort((a, b) => {
      const ka = key(a), kb = key(b);
      return ka < kb ? -1 : ka > kb ? 1 : 0; // sort by date, then time
    });
  }
...
function renderCard(card, list) {
  ...
  // Interview date + time (only in the "For Interview" list): a mini
  // macOS-Calendar-style icon (red month header + big day number) paired with
  // the time. Each is a transparent native input overlaying its visual, so a
  // TAP opens the OS picker directly (reliable on mobile), and a click also
  // calls showPicker() (so desktop opens too). Defaults: upcoming Sunday, 9 AM.
  if (list.title === INTERVIEW_LIST) {
    const iso = effectiveInterviewDate(card);
    const [Y, M, D] = iso.split('-').map(Number);
    const monthAbbr = new Date(Y, M - 1, D).toLocaleDateString(undefined, { month: 'short' }).toUpperCase();

    // A picker = visual element with a transparent input laid over it.
    const makePicker = (visualEl, input, onCommit) => {
      input.addEventListener('change', () => { onCommit(input.value); save(); render(); });
      // Don't let taps start a card drag or open the card.
      ['pointerdown', 'touchstart'].forEach(ev =>
        input.addEventListener(ev, (e) => e.stopPropagation()));
      input.addEventListener('click', (e) => {
        e.stopPropagation();
        try { input.showPicker(); } catch (_) {} // enhancement (desktop); tap already opens on mobile
      });
      visualEl.appendChild(input);
      return visualEl;
    };
    ...
    // Date — the calendar icon
    const cal = document.createElement('div');
    cal.className = 'cal-icon';
    cal.title = 'Interview date — tap to change';
    cal.innerHTML = `<span class="cal-month">${monthAbbr}</span><span class="cal-day">${D}</span>`;
    const dinput = document.createElement('input');
    dinput.type = 'date'; dinput.value = iso; dinput.className = 'cal-input';
    dinput.setAttribute('aria-label', 'Interview date');
    when.appendChild(makePicker(cal, dinput, (v) => { card.interviewDate = v || ''; }));

    // Time
    const time = effectiveInterviewTime(card);
    const timeEl = document.createElement('div');
    timeEl.className = 'when-time';
    timeEl.title = 'Interview time — tap to change';
    timeEl.innerHTML = `<span class="when-time-txt">${format12h(time)}</span>`;
    const tinput = document.createElement('input');
    tinput.type = 'time'; tinput.value = time; tinput.className = 'when-input';
    tinput.setAttribute('aria-label', 'Interview time');
    when.appendChild(makePicker(timeEl, tinput, (v) => { card.interviewTime = v || ''; }));
