...
function renderList(list) {
  ...
  if (list.title === INTERVIEW_LIST) {
    list.cards.sort((a, b) => {
      const da = effectiveInterviewDate(a), db = effectiveInterviewDate(b);
      return da < db ? -1 : da > db ? 1 : 0; // ISO strings sort chronologically
    });
  }
...
function renderCard(card, list) {
  ...
  // Interview date chip (only in the "For Interview" list). Defaults to the
  // upcoming Sunday; tap to pick another date. A transparent date input
  // overlays the chip so tapping opens the native date picker.
  if (list.title === INTERVIEW_LIST) {
    const iso = effectiveInterviewDate(card);
    const chip = document.createElement('div');
    chip.className = 'card-date';
    chip.title = 'Interview date — tap to change';
    chip.innerHTML = iconCalendar() + `<span class="card-date-txt">${formatDateShort(iso)}</span>`;
    const dinput = document.createElement('input');
    dinput.type = 'date';
    dinput.value = iso;
    dinput.className = 'card-date-input';
    dinput.setAttribute('aria-label', 'Interview date');
    dinput.addEventListener('change', () => {
      card.interviewDate = dinput.value || '';
      save(); render();
    });
    chip.appendChild(dinput);
    ['pointerdown', 'click', 'touchstart'].forEach(ev =>
      chip.addEventListener(ev, (e) => e.stopPropagation())); // don't drag/open the card
...
function iconCalendar() { return '<svg viewBox="0 0 24 24"><path d="M7 2v2H5a2 2 0 00-2 2v13a2 2 0 002 2h14a2 2 0 002-2V6a2 2 0 00-2-2h-2V2h-2v2H9V2H7zm12 7v10H5V9h14z"/></svg>'; }
